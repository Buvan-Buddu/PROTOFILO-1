...
toggleButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        // 2. Filter Skill Cards with staggered animation
        ...
        // 3. Filter Tech Icon Rows (the 🌐 HTML, 🟢 Node.js list at the bottom)
        techIconContainers.forEach(container => {
            if (container.classList.contains(`${category}-icons`)) {
                container.classList.remove('hidden');
                container.style.display = 'flex';
                container.style.animation = 'fadeIn 0.5s ease-out 0.5s forwards';
            } else {
...
// Initialize: Ensure Frontend is visible on page load
window.addEventListener('DOMContentLoaded', () => {
    // Simulate a click on the frontend button to set initial state
    const frontendBtn = document.querySelector('[data-category="frontend"]');
